...
function getStatusIcon(status) {
  ...
  if (status === "Out for Delivery") return <FaShippingFast />;
  if (status === "In Transit") return <FaTruckLoading />;
  return <FaRoute />;
}

function getRoutePoint(status) {
  if (status === "Delivered") return { x: 370, y: 120 };
  if (status === "Out for Delivery") return { x: 320, y: 78 };
  if (status === "In Transit") return { x: 245, y: 104 };
  return { x: 90, y: 170 };
}
  ...
  const liveRouteLabel =
    trackingInfo?.status === "Delivered"
      ...
      : nextRouteEvent
        ? "Next Stop"
        : "Current Stop";
  const liveRouteValue =
    trackingInfo?.status === "Delivered"
      ? trackingInfo.destination
      : nextRouteEvent?.location || trackingInfo?.currentLocation;
  const routePoint = getRoutePoint(trackingInfo?.status);
  const liveStatusIcon = getStatusIcon(trackingInfo?.status);

  return (
    <main className="min-h-screen bg-[#F8FAFC] text-[#2F343A]">
      <section className="relative overflow-hidden border-b border-[#F68B1F]/10 bg-gradient-to-br from-white via-[#FFF8EF] to-[#F3F5F7]">
        <div className="absolute inset-x-0 top-0 h-80 bg-gradient-to-br from-[#FFF3E4] via-white to-[#F3F5F7]" />
        <div className="absolute left-0 top-0 h-72 w-72 rounded-full bg-[#F68B1F]/15 blur-[100px]" />
        <div className="absolute right-0 top-16 h-72 w-72 rounded-full bg-[#FFD700]/20 blur-[100px]" />

        <div className="relative mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 px-5 py-12 sm:px-8 lg:grid-cols-[minmax(0,1fr)_minmax(500px,0.9fr)] lg:gap-12 lg:py-16 xl:grid-cols-[minmax(0,1fr)_minmax(540px,0.82fr)]">
          <div className="max-w-3xl">
            ...
            <form
              ...
              className="mt-8 rounded-[26px] border border-[#F68B1F]/20 bg-white p-3 shadow-[0_20px_60px_rgba(246,139,31,0.12)]"
            >
              <div className="flex flex-col gap-3 rounded-[20px] bg-[#FFF8EF] p-2 sm:flex-row">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="h-14 w-full rounded-2xl border border-[#F68B1F]/10 bg-white pl-12 pr-4 text-sm font-bold text-[#2F343A] outline-none transition placeholder:text-[#9AA2AA] focus:border-[#F68B1F]/60 focus:ring-4 focus:ring-[#F68B1F]/15"
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="inline-flex h-14 cursor-pointer items-center justify-center gap-3 rounded-2xl bg-gradient-to-r from-[#F68B1F] to-[#FFD700] px-7 text-sm font-bold text-white shadow-[0_14px_30px_rgba(246,139,31,0.28)] transition hover:-translate-y-0.5 hover:from-[#E76F00] hover:to-[#F68B1F] disabled:cursor-not-allowed disabled:opacity-70"
                >
                  {isLoading ? (
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {orders.map((order) => (
                  <button
                    ...
                    className="rounded-full border border-[#D9DEE3] bg-white px-4 py-2 text-xs font-bold text-[#6F7780] transition hover:-translate-y-0.5 hover:border-[#F68B1F] hover:text-[#F68B1F] hover:shadow-sm"
                  >
                    {order.id}
                  </button>
                ))}
          ...
          <div className="mx-auto w-full max-w-[620px] lg:max-w-none">
            <div className="overflow-hidden rounded-[30px] border border-[#F68B1F]/15 bg-white shadow-[0_28px_80px_rgba(47,52,58,0.10)]">
              <div className="border-b border-[#F68B1F]/10 bg-gradient-to-r from-white via-[#FFF8EF] to-[#FFF3E4] p-5 lg:p-6">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#F68B1F]">
                      Shipment Console
                    </p>
                    <h3 className="mt-2 text-2xl font-bold text-[#2F343A]">
                      Invictus Logistics
                    </h3>
                  </div>

                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-[#F68B1F] shadow-sm">
                    <FaBell />
                  </div>
                </div>
              </div>
              ...
              <div className="p-5 lg:p-6">
                <div className="rounded-[24px] border border-[#D9DEE3] bg-[#FFF8EF] p-5 lg:p-6">
                  <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <p className="text-sm font-bold text-[#6F7780]">Current Order</p>
                      <h4 className="mt-1 text-2xl font-bold text-[#2F343A]">
                        {trackingInfo?.status || "Ready to Track"}
                      </h4>
                    </div>

                    <span className="w-fit rounded-full bg-[#F68B1F] px-4 py-2 text-xs font-bold text-white">
                      Live
                    </span>
                  </div>

                  <p className="mb-5 rounded-2xl bg-white px-4 py-3 text-xs font-bold text-[#9AA2AA]">
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="rounded-[22px] bg-gradient-to-r from-[#F68B1F] to-[#FFD700] p-5 text-white sm:flex sm:items-center sm:gap-4">
                    <FaShippingFast className="mb-3 text-3xl sm:mb-0 sm:text-4xl" />
                    <div>
                      <p className="text-xs font-bold text-white/80">Progress</p>
                      <p className="mt-1 text-lg font-bold">{progress}% Done</p>
                    </div>
                  </div>
                </div>

                <div className="mt-5 flex items-center justify-between rounded-[24px] border border-[#D9DEE3] bg-[#FFF8EF] p-3">
      ...
      {isLoading && (
        <section className="px-5 pb-10 sm:px-8">
          <div className="mx-auto max-w-[1180px] rounded-[34px] border border-[#F68B1F]/20 bg-white/90 p-8 text-center shadow-[0_18px_60px_rgba(246,139,31,0.10)]">
            ...
            <h3 className="text-2xl font-bold text-[#2F343A]">
              Tracking Shipment
            </h3>
              ...
              <div className="grid gap-6 bg-gradient-to-r from-[#FFF3E4] via-white to-[#FFF8EF] p-6 md:grid-cols-[1fr_auto] md:p-8">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <code className="rounded-full bg-white px-4 py-2 text-sm font-bold text-[#F68B1F] shadow-sm">
                      {trackingInfo.id}
                    </code>
                ...
                <div className="rounded-[24px] border border-[#F68B1F]/20 bg-white p-5 text-center shadow-sm">
                  ...
                  <h3 className="mt-1 text-4xl font-extrabold text-[#F68B1F]">
                    {progress}%
                  </h3>
              ...
              <div className="p-6 md:p-8">
                ...
                <div className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
                  ...
                  <div className="rounded-[32px] border border-[#F68B1F]/20 bg-[#FFF8EF] p-5 sm:p-6">
                    <h3 className="mb-5 text-xl font-bold text-[#2F343A]">
                      Shipment Details
                    </h3>
                    ...
                    <div className="grid gap-4">
                      ...
                      <DetailRow icon={liveStatusIcon} label="Live Status" value={trackingInfo.status} active />
                    </div>
                    ...
                    <div className="mt-6 grid grid-cols-2 gap-3">
                      <button
                        ...
                        className="flex items-center justify-center gap-2 rounded-2xl border border-[#D9DEE3] bg-white px-4 py-3 text-sm font-bold text-[#2F343A] transition hover:border-[#F68B1F] hover:text-[#F68B1F]"
                      >
                        <FaPrint />
                        Print
                      </button>
                      ...
                      <button
                        type="button"
                        onClick={handleShare}
                        className="flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#F68B1F] to-[#FFD700] px-4 py-3 text-sm font-bold text-white transition hover:from-[#E76F00] hover:to-[#F68B1F]"
                      >
                        <FaShare />
                        Share
                      </button>
                        ...
                        <div key={index} className="relative flex gap-4">
                          ...
                          >
                            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                              <h4 className="font-bold text-[#2F343A]">
                                {event.status}
                              </h4>
                ...
                <div className="mt-8 rounded-[28px] border border-[#F68B1F]/20 bg-[#FFF8EF] p-5 sm:flex sm:items-center sm:justify-between sm:gap-5">
                  ...
                  <button
                    ...
                    className="mt-4 w-full rounded-2xl bg-[#2F343A] px-6 py-3 text-sm font-bold text-white transition hover:bg-[#F68B1F] sm:mt-0 sm:w-auto"
                  >
                    Contact Support
                  </button>
                </div>
      ...
      {!trackingInfo && !isLoading && (
        <section className="px-5 pb-20 sm:px-8">
          <div className="mx-auto max-w-[1180px] rounded-[34px] border border-[#F68B1F]/20 bg-white/90 p-8 text-center shadow-[0_18px_60px_rgba(246,139,31,0.08)]">
            ...
            <h3 className="text-1xl font-bold text-[#2F343A]">
              Track your shipment
            </h3>
      ...
      {showPopup && (
        <div className="fixed bottom-5 right-5 z-50 rounded-2xl bg-white px-5 py-4 text-sm font-bold text-[#2F343A] shadow-2xl border border-[#F68B1F]/20">
...
function SmallPreviewCard({ icon, title, label, value }) {
  return (
    <div className="rounded-[22px] border border-[#D9DEE3] bg-white p-4 lg:flex lg:items-center lg:gap-4">
      <div className="mb-3 text-3xl text-[#F68B1F] lg:mb-0 lg:text-4xl">{icon}</div>
      <div>
        <p className="text-sm font-bold text-[#2F343A]">{title}</p>
        <p className="mt-2 text-xs text-[#9AA2AA]">{label}</p>
        <p className="mt-1 text-sm font-bold text-[#2F343A]">{value}</p>
      </div>
    </div>
  );
}
...
function InfoCard({ icon, label, value, helper }) {
  return (
    <div className="rounded-[26px] border border-[#F68B1F]/20 bg-[#FFF8EF] p-5 transition hover:-translate-y-1 hover:border-[#F68B1F]/50 hover:bg-white hover:shadow-[0_18px_45px_rgba(246,139,31,0.12)]">
      ...
      <h3 className="mt-1 break-words text-lg font-bold text-[#2F343A]">
        {value}
      </h3>
...
function DetailRow({ icon, label, value, active = false }) {
  return (
    <div
      className={`flex items-center justify-between gap-4 rounded-2xl p-4 shadow-sm transition hover:-translate-y-0.5 ${
        active
          ? "border border-[#F68B1F]/30 bg-[#FFF8EF]"
    ...
    >
      <div className="flex items-center gap-3">
        <div
          className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${
            active
              ? "bg-gradient-to-r from-[#F68B1F] to-[#FFD700] text-white"
              : "bg-[#F68B1F]/10 text-[#F68B1F]"
          }`}
        >
          {icon}
        </div>
      ...
      <span className="text-right text-sm font-bold text-[#2F343A]">
        {value}
      </span>
    </div>
  );
